...
keywords:
  ...
  - Plugin
  - MongoDB Logger
description: 本文介绍了 API 网关 Apache APISIX 如何使用 mongodb-logger 插件将访问日志直接写入 MongoDB 集合。
---
...
`mongodb-logger` 插件通过原生 MongoDB Wire 协议（TCP/TLS 上的 OP_MSG）将 APISIX 访问日志直接写入 [MongoDB](https://www.mongodb.com/) 集合。每条访问日志以结构化 BSON 文档的形式存储，与所有 APISIX 日志插件通用的标准日志格式保持一致。

主要特性：

- **原生 Wire 协议** — 直接通过 MongoDB 二进制协议通信，无需 HTTP 中间层。
- **SCRAM 身份认证** — 支持 SCRAM-SHA-1（默认）和 SCRAM-SHA-256（需要 `lua-resty-openssl`），完整实现双向认证（按 RFC 5802 验证服务端签名）。
- **TLS/SSL** — 可选加密传输，支持配置服务端证书验证。
- **副本集 / Kubernetes 支持** — `hosts` 数组允许配置多个 `host:port` 端点；插件每批次随机选取一个端点，失败时依次尝试其余端点，天然适配副本集成员和 Kubernetes Headless Service。
- **连接池** — 通过 nginx cosocket keepalive 池在请求间复用 TCP 连接（及 TLS 会话），避免重复握手开销。
- **批量写入** — 日志条目在内存中积累后批量刷写，最小化与 MongoDB 的往返次数。
- **无序插入（`ordered=false`）** — 单条文档写入失败不中止整批；各文档级错误以 APISIX 警告日志的形式上报。

## 属性

| 名称                   | 类型    | 必选项 | 默认值              | 有效值                              | 描述 |
|------------------------|---------|--------|---------------------|-------------------------------------|------|
| host                   | string  | 否     | `"127.0.0.1"`       |                                     | MongoDB 主机地址。设置 `hosts` 后忽略此字段。 |
| port                   | integer | 否     | `27017`             | [1, 65535]                          | MongoDB 端口。设置 `hosts` 后忽略此字段。 |
| hosts                  | array   | 否     |                     | `["host:port", ...]`                | MongoDB 端点列表。配置后，插件每批次随机选取一个端点，失败时依次尝试其余端点。适用于副本集和 Kubernetes Headless Service。 |
| database               | string  | 是     |                     |                                     | 目标数据库名称（不能为空）。 |
| collection             | string  | 是     |                     |                                     | 目标集合名称（不能为空）。 |
| username               | string  | 否     |                     |                                     | MongoDB 用户名。留空则跳过认证。 |
| password               | string  | 否     |                     |                                     | MongoDB 密码。在 etcd 中加密存储（详见下方 `encrypt_fields` 说明）。 |
| auth_database          | string  | 否     | `"admin"`           |                                     | 存放用户凭据的认证数据库（通常为 `admin`）。 |
| auth_mechanism         | string  | 否     | `"SCRAM-SHA-1"`     | `"SCRAM-SHA-1"`、`"SCRAM-SHA-256"` | SCRAM 认证机制。`SCRAM-SHA-256` 需要 `lua-resty-openssl`。 |
| ssl                    | boolean | 否     | `false`             |                                     | 启用 TLS/SSL。 |
| ssl_verify             | boolean | 否     | `true`              |                                     | `ssl` 为 `true` 时是否验证服务端证书。 |
| timeout                | integer | 否     | `3000`              | [1, ...]                            | Socket 超时时间（毫秒）。 |
| pool_size              | integer | 否     | `5`                 | [1, ...]                            | 每个 nginx worker 在 cosocket keepalive 池中保持的最大空闲连接数。 |
| keepalive_timeout      | integer | 否     | `60`                | [1, ...]                            | 空闲连接的 keepalive 超时时间（秒）。 |
| name                   | string  | 否     | `"mongodb-logger"`  |                                     | 日志器唯一名称，会在 Prometheus 指标 `apisix_batch_process_entries` 中上报。 |
| log_format             | object  | 否     |                     |                                     | 以键值对形式声明的 JSON 日志格式。值支持字符串和嵌套对象；字符串中可通过 `$` 前缀引用 [APISIX](../apisix-variable.md) 或 [NGINX](http://nginx.org/en/docs/varindex.html) 变量。 |
| include_req_body       | boolean | 否     | `false`             |                                     | 为 `true` 时在日志中记录请求体。请求体过大无法在内存中缓存时将无法记录（nginx 限制）。 |
| include_req_body_expr  | array   | 否     |                     |                                     | [lua-resty-expr](https://github.com/api7/lua-resty-expr) 过滤器：仅在表达式为 `true` 时记录请求体。需同时开启 `include_req_body = true`。 |
| include_resp_body      | boolean | 否     | `false`             |                                     | 为 `true` 时在日志中记录响应体。 |
| include_resp_body_expr | array   | 否     |                     |                                     | 针对 `include_resp_body` 的 [lua-resty-expr](https://github.com/api7/lua-resty-expr) 过滤器。 |

:::note 说明

插件 schema 中定义了 `encrypt_fields = {"password"}`，表示 `password` 字段在 etcd 中将以加密方式存储。详见[加密存储字段](../plugin-develop.md#encrypted-storage-fields)。

:::

本插件支持使用批处理器对日志条目进行聚合批量处理，避免频繁提交数据。批处理器每隔 `5` 秒或队列积累达到 `1000` 条时自动提交。更多信息或自定义配置请参考[批处理器](../batch-processor.md#configuration)。

### 默认日志格式

每条日志以 BSON 文档的形式存储，遵循 APISIX 统一日志格式：
...
您也可以通过配置插件元数据来统一设置日志格式，可用配置如下：

| 名称                | 类型    | 必选项 | 默认值 | 描述 |
|---------------------|---------|--------|--------|------|
| log_format          | object  | 否     |        | 以键值对形式声明的 JSON 日志格式。值支持字符串和嵌套对象；字符串中可通过 `$` 前缀引用 [APISIX](../apisix-variable.md) 或 [NGINX](http://nginx.org/en/docs/varindex.html) 变量。 |
| max_pending_entries | integer | 否     |        | 批处理器开始丢弃条目前，允许缓冲的最大待处理条目数。 |

:::info 重要

插件元数据的配置为全局生效，会影响所有使用 `mongodb-logger` 插件的路由和服务。
...
可以使用以下命令从 `config.yaml` 中获取 `admin_key` 并保存为环境变量：
...
### 副本集 / Kubernetes Headless Service

在 `hosts` 数组中填写所有副本集成员地址（或 Headless Service 的全部 Pod IP）。插件每批次随机选取一个端点；若该端点不可达，将依次尝试其余端点，直至批次成功发送或全部端点均失败。这一机制可在副本集成员间实现无配置负载均衡，并在 MongoDB Pod 滚动重启时保持可用性。
...
在 Kubernetes 环境中，敏感信息（用户名、密码）可通过 APISIX 的 `${{ENV_VAR}}` 语法从 Kubernetes Secret 对象注入：将密钥以环境变量的形式挂载到 APISIX Pod，再在插件配置中引用即可。
...
| 场景 | 建议 |
|------|------|
| 高吞吐、延迟不敏感 | 增大 `batch_max_size`（如 5000），增大 `batch_max_wait` |
...
如需移除 `mongodb-logger` 插件，在插件配置中删除对应的 JSON 配置即可。APISIX 会自动加载，无需重启。
